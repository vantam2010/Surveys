...
final class SurveyService: SurveyServiceProtocol {
    ...
    func fetchData(paging: Paging, completion: @escaping ((Result<[Survey], ErrorResult>) -> Void)) {
        ...
        task = networking.makeRequest(resource: Resource.init(baseUrl: Configuration.baseUrl, path: "surveys.json", method: .get, params: paging.dictionary ?? [:]), completion: completion)
    }
